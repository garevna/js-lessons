# ![ico-30 icon] Условные операторы⟪uslovnie_operatori⟫

Сасо название "условные операторы" уже говорит нам о том, что они отличаются от всех остальных операторов тем, что в них будет какое-то **условие**.
Условия часто встречаются в нашей повседневной жизни.
"Ты пойдешь в кино при условии, что выполнишь домашнее задание".
Очевидно, что эта фраза предполагает два варианта: либо я пойду в кино, либо нет.
И выбор одного из двух этих вариантов зависит от выполнения или невыполнения условия: выполнение домашнего задания.

Очевидно, мы столкнулись с **ветвлением**.
...
## ![ico-25 icon] Оператор if⟪operator_if⟫
...
Какое бы выражение вы не поместили в круглые скобки, движок вычислит его значение и приведет его к типу ~boolean~, т.е. в круглых скобках может быть любое синтаксически верное выражение, но в результате вычисления значения этого выражения движок получит либо **~true~**, либо **~false~**.
Позже мы познакомимся с 
...
Осталось заменить **~Если~** на **~if~** и **~в противном случае~** на **~else~**:
...
В этой записи появляется две ветки кода: _~скрипт 1~_ и _~скрипт 2~_

Какая ветка кода будет выполнена, зависит от значения логического выражения в круглых скобках оператора **if**

Если значением логического выражения будет ~true~, то выполняться будет _~скрипт 1~_

В противном случае (если значением логического выражения будет ~false~ ) выполняться будет _~скрипт 2~_
...
## ![ico-25 icon] Тернарный оператор⟪ternarnii_operator⟫
...
#### ![ico-30 cap] Example 3⟪Example_3⟫
...
※※※exercises test/conditionalOperators※※※
