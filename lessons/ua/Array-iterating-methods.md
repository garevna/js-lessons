# ![ico-35 study] Ітеруючі методи масивів⟪Array_iterating_methods⟫
...
### ![ico-25 icon] Контекст виклику функції-аргументу⟪Function-argument_call_context⟫
...
### ![ico-20 icon] Приклади з методом map⟪Examples_with_the_~map~_method⟫
...
В іншому випадку повертає число - свій порядковий номер (0, 1, 2 ...).
...
В цьому прикладі ми створюємо ще один масив функцій з іменами "plus", "minus", "divide", "multiply".
Кожна з цих функцій має два формальних параметри,
Тому перш за все вона перевіряє довжину об'єкта arguments,
Якщо довжина дорівнює 2, то виконує відповідну операцію з аргументами
(додає, віднімає, множить, ділить).
В іншому випадку повертає каррированну функцію,
у якої перший аргумент вже "прошитий",
...
Функція-аргумент методу повинна повертати нове значення (повинен бути присутній оператор ~return~).
...
Так само, як і в методі **~forEach()~**, у методі **~map()~** функція-аргумент має три опціональних формальних параметри.
![ico-20 green-ok] Аргумент **arr** буде містити посилання на початковий масив.
![ico-20 green-ok] Аргумент **index**  - це лічильник ітерацій, або індекс поточного елемента ітерованого масиву.
...
### ![ico-20 icon] Приклади з методом `map`⟪Examples_using_the_-map-_method⟫

Перейдіть по [![ico-20 link] **_посиланню_**](https://developer.mozilla.org/en-US/docs/Web/API/Window/location?name=garevna,date=10.07.2018)
...
У результуючий масив потраплять тільки ті елементи, які задовольняють умові фільтрації.
...
Метод шукає в масиві і повертає перший знайдений елемент, що задовольняє задану умову.

Якщо такого елемента в масиві немає, повертає ~undefined~.
...
Подібно методу ~find()~, шукає в масиві перший елемент, що задовольняє задану умову.
Однак повертає не сам елемент, а його індекс.

Якщо такого елемента в масиві немає, повертає **-1**.
...
Очевидна перевага перед методом **~indexOf~** полягає в тому, що можна працювати з масивами елементів, які мають **посилальний тип даних**.
У цьому прикладі елементами масиву є об'єкти, і метод **~indexOf~** до цього масиву непридатний.
...
Перевіряє масив на предмет входження елементів, **не** задовольняючих задану умову.

Повертає логічне значення:
  • якщо всі елементи масиву благополучно пройшли перевірку - **~true~**.
  • якщо хоча б один елемент не пройшов перевірку - **~false~**.

Функція, що передається методу як перший аргумент, перевіряє виконання заданої умови для кожного елемента масиву і повертає логічне значення.

Масив ітерується до тих пір, поки функція не поверне значення **~false~**.
У цьому випадку метод поверне **~false~**.

Якщо функція поверне **~true~** для всіх елементів масиву, метод поверне **~true~**.
...
^^Змінна  **res**  матиме значення ~false~, оскільки в масиві є елементи, які не задовольняють задану умову.^^

______________________________________________

## ![ico-25 icon] some()⟪some⟫

Перевіряє масив на наявність елементів, які задовольняють задану умову.
...
Масив проходить ітерацію доти, доки функція не поверне значення **~true~**.
...
^^У цьому прикладі масив **people** перевіряється на наявність у ньому мешканців Пакистану.^^
^^Змінна **res** матиме значення ~false~, оскільки таких «персонажів» у масиві немає.^^

^^Метод **some** можна замінити таким кодом:^^
...
А саме — першим формальним параметром **функції-аргументу** тепер буде не поточний елемент масиву, а **змінна-акумулятор**.
Значення цієї змінної і буде результатом роботи методу.

Гнучкість та універсальність цього методу полягає в тому, що результат його роботи може бути чим завгодно: числом, масивом, об’єктом, рядком, логічним значенням тощо.
Цей результат «накопичується» у **змінній-акумуляторі**.

Як і всі вже розглянуті ітераційні методи масивів, метод **~reduce~** як перший обов’язковий аргумент отримує функцію.
Однак другий аргумент методу не є посиланням на контекст виклику **функції-аргументу**.
Він має зовсім інше призначення: він задає початкове значення **змінної-акумулятора**.

![ico-20 warn] **Якщо початкове значення акумулятора не встановлено, то як початкове значення акумулятора буде використано значення першого елемента масиву**.

^^Тобто якщо ми не передамо методу другий аргумент, то він встановить початкове значення **змінної-акумулятора** рівним значенню першого елемента масиву, що ітерується.^^
...
Оскільки початкове значення акумулятора не задано, то воно дорівнюватиме значенню першого елемента масиву, тобто ** 1**.
Цикл буде «крутитися» до останнього елемента масиву, але самі елементи не використовуються **функцією-аргументом**:

~~~js
accumulator => accumulator * 2
~~~

Тому на кожній ітерації значення **змінної-акумулятора** просто подвоюватиметься:

•• 1 * 2 * 2 * 2 * 2 = 16 ••

_____________________________________

![ico-25 cap] Тепер запросимо на цю вечірку другий формальний параметр **функції-аргументу**:

~~~js
[1, 2, 3, 4, 5].reduce((accumulator, item) => accumulator * item) // 120
~~~

Початкове значення **змінної-акумулятора** дорівнюватиме ** 1** ^^(значення першого елемента масиву)^^, але на кожній ітерації **змінна-акумулятор** буде множитися на значення поточного елемента масиву:

•• 1 * 2 * 3 * 4 * 5 = 120 ••

____________________________

![ico-25 cap] Час ввести в дію початкове значення **змінної-акумулятора**:
...
Ось така чудова машинка тепер у нашому розпорядженні.

_________________________________

![ico-25 cap] Заглибимося далі. Задіємо третій формальний параметр **функції-аргументу**:
...
Досі ми розглядали масив чисел і числове значення **змінної-акумулятора**.

Однак потенціал цього методу набагато більший, а його можливості — набагато ширші.

### ![ico-20 icon] Приклади з методом reduce()⟪Examples_using_the_reduce_method⟫

Давайте зведемо масив рядків до об’єкта.
Для цього нам обов’язково потрібно вказати початкове значення акумулятора, оскільки значення першого елемента масиву буде рядком, а ми хочемо отримати об’єкт.
...
Тепер підрахуємо, скільки разів кожен символ зустрічається в рядку, і повернемо об’єкт:
...
Тепер підрахуємо, скільки мешканців кожної країни в списку.
...
Движок використовує експоненційну форму представлення малих чисел, тобто замість **~0.0000005~** буде **~5e-7~**.
Для великих чисел відбувається те ж саме, т.е. замість **~5000000000000000000000~** буде **~5e+21~**.
...
як і в разі інших ітераційних методів, може приймати додаткові аргументи — індекс поточного елемента масиву та посилання на сам вихідний масив.
Завдяки посилання ми можемо маніпулювати вихідним масивом, що робить його мутабельним.
...
Простежимо, як працюватиме метод **~reduce~**, якщо передати йому як перший аргумент **~Math.sqrt~**.
Бібліотечна функція **~Math.sqrt~** приймає лише 1 аргумент (число) і повертає квадратний корінь з отриманого аргументу.
Якщо при виклику ми не передаємо методу **~reduce~** другий аргумент, він буде використовувати як початкове значення акумулятора значення першого елемента масиву.
На кожній ітерації це значення буде замінюватися його квадратним коренем.
Інші елементи масиву в обчисленнях брати участь не будуть, оскільки **~Math.sqrt~** приймає всього 1 аргумент, і це буде поточне значення змінної-акумулятора.
...
У цьому прикладі з числа 625 двічі був витягнутий квадратний корінь:
••625 -> 25 -> 5••

Чому двічі? Тому що всього елементів у масиві 3, але перший елемент став початковим значенням змінної-акумулятора.

Якщо ми додамо ось такий метод **~root~** у прототип конструктора **~Array~**, то результат його виклику буде той самий:
...
У цьому прикладі ми задаємо початкове значення **змінної-акумулятора** (625 * 625) другим аргументом методу **~reduce~**.
Тому кількість ітерацій (тобто скільки разів буде витягнутий квадратний корінь із **змінної-акумулятора**) дорівнюватиме кількості елементів масиву.
...
Тепер будемо передавати методу **~reduce~** першим аргументом бібліотечну функцію **Math.pow** (піднесення до степеня).
Ця функція приймає два числових аргументи: число, яке потрібно піднести до степеня, і значення степеня.

Якщо ми не передаємо початкове значення **змінної-акумулятора** другим аргументом методу **~reduce~**, то в цьому якості буде використано значення першого елемента масиву.
Інші елементи масиву будуть значеннями степеня, у який потрібно піднести поточне значення **змінної-акумулятора**.

Якщо ж ми передаємо початкове значення **змінної-акумулятора**, то всі елементи масиву будуть розглядатися як степінь, у який потрібно піднести поточне значення **змінної-акумулятора**.
...
Якщо ми додамо в прототип конструктора **~Array~** метод **~pow~**, то цей метод буде працювати аналогічно методу **~reduce~**, викликаному з бібліотечною функцією **~Math.pow~** і без другого аргументу:
...
Кількість ітерацій буде заведомо більшою, ніж кількість елементів масиву.

Метод сортує масив відповідно до заданого умови сортування.
Умова сортування перевіряється передаваною методу функцією-аргументом.
_Функція-аргумент має два формальних параметра, значення яких використовуються для порівняння._.

Функція повертає одне з трьох значень:

|  0 | елементи збіглися (рівні)      |
|  1  | перший аргумент більший за другий      |
| -1  | другий аргумент більший за перший     |

На основі значення, яке повертає функція, метод змінює порядок елементів у масиві.
...
^^Щоб зрозуміти механізм сортування, виведемо в консоль значення порівнюваних елементів масиву на кожній ітерації:^^
...
^^^[Протоколювання]
^^Створимо масив **log**, у якому будемо записувати всі операції під час сортування масиву.^^
~~~js
var log = []
var resArray = sourceArray
  .sort(function (x, y) {
...
Отже, на відміну від інших методів ітерації, функція, що передається методу як єдиний аргумент, приймає саме два параметри.

![ico-20 warn] Цьому методу не можна передати посилання на контекст виклику.

![ico-20 require] Спробуйте самостійно намалювати блок-схему алгоритму сортування масиву методом  **~sort()~**.
...
Тепер застосуємо метод **~flatMap~** до масиву **cookie**
...
Отже, використовуючи одну й ту саму функцію:
...
у першому випадку ми отримали масив масивів, а в другому — «плоский» масив
...
## ![ico-30 icon] Приклади та тести⟪Examples_and_tests⟫

_________________________________________

### ![ico-25 cap] Сума кешу на картах⟪Cache_amount_on_cards⟫

Створимо масив банківських карток і за допомогою методу **~reduce~** підрахуємо суму коштів на всіх картках:
...
Створимо нову картку, на яку акумулюємо залишки на рахунках всіх карток:
...
### ![ico-25 cap] Очищення від дублікатів⟪Clearing_duplicates⟫

Припустимо, що у нас є масив користувачів, і ми вирішили отримати масив усіх спеціальностей, які є у цих користувачів.
~~~js
const users = [
...
Однак, як ми бачимо, багато спеціальностей будуть дублюватися, що нам абсолютно не потрібно.
...
Потрібно видалити дублюючі елементи масиву **~specialities~**.
...
Але ми вирішили піти більш складним шляхом ![ico-25 wink].
...
### ![ico-25 cap] Кількість відрізків в інтервалі⟪Number_of_nested_segments⟫
...
~~~
Тепер можна порівняти два відрізки дуже просто:
~~~js
...
### ![ico-25 cap] Перевірка парних дужок⟪Checking_paired_brackets⟫
