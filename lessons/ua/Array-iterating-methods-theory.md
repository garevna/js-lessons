# Принцип роботи ітераційних методів⟪Deeper_into_iterating_methods⟫
...
^^(додають елемент до масиву, видаляють або замінюють елементи в масиві, об’єднують кілька масивів в один тощо).^^
...
## ![ico-25 icon] Декоратор циклу⟪Cycle_decorator⟫
...
## ![ico-25 icon] Передача посилань⟪Transferring_references⟫

Тут, на жаль, закінчується функціональна чистота ітеруючих методів.
**Посилання — це «відмичка»**, і за допомогою посилання функція може змінювати об’єкт.
Як правило, це призводить до побічних ефектів (**side effects**).

Є два способи передачі посилань в ітеруючих методах масивів.
Перший — передача посилання на контекст виклику функції-аргументу як другого аргументу методу.
Другий — передача посилання на вихідний масив самій функції-аргументу під час виклику.

### ![ico-20 icon] Другий аргумент методу⟪The_second_argument_of_the_method⟫
...
Однак з’являється побічний ефект — масив ~alter~ тепер порожній:
...
### ![ico-20 icon] Передача посилання на вихідний масив функції-аргументу⟪Passing_a_reference_to_the_original_array_to_the_function-argument⟫
...
Передача цієї "відмички" функції-аргументу грубо руйнує функціональну чистоту методу і створює величезні можливості для побічних ефектів (**side effects**).
...
^^Функція **sample** отримує в змінній **_arr_** посилання на вихідний масив **numbers**.^^

___________________________________________________

## ![ico-25 icon] Side effects⟪Side_effects⟫

![ico-20 warn] Чисті функції, якими, за задумом, мали бути ітераційні методи, не спричиняють зовнішніх ефектів, тобто не змінюють жодних зовнішніх змінних.
Методи ітерації масивів — це **функції вищого порядку**, які перебирають вихідний масив, передаючи функції-аргументу по черзі один елемент масиву за іншим.
Таким чином, функція-аргумент не має посилання на сам вихідний масив, і не може його модифікувати.
_______________________
![ico-20 warn] Виняток — якщо елементи вихідного масиву мають **посилальний тип даних**, тобто функція-аргумент отримує посилання на елемент масиву.
______________________
Завжди повертається новий масив (або не масив, як ми побачимо пізніше, або взагалі нічого).
Вихідний масив, як правило, не змінюється.
Тобто спочатку це мали бути чисті функції, що не породжують зовнішніх ефектів, що відрізняє їх від звичайних методів масивів, таких як ~push()~, ~concat()~ тощо.

Але ми вже показали раніше, що ця функціональна чистота порушується передачею посилань (методу — на контекст виклику функції-аргументу та на вихідний масив — функції-аргументу).

Аналогічно, якщо ми маємо справу з **глибокими структурами даних**, то відбувається передача не значень, а посилань, що створює можливість виникнення побічних ефектів.
...
У цьому прикладі ми ітеруємо масив об'єктів, тобто маємо справу з глибокими структурами даних.
...
У результаті виконання коду масив **users** матиме такий вигляд:
...
Ми бачимо, що вік усіх користувачів змінився, хоча ми не передавали посилання на вихідний масив **users**.
Але масив **users** є **глибоким структурою даних**, і його елементи мають **тип посилання**,
тому функція-аргумент (~user => user.age++~) методу **~iterate~** отримала "відмичку" і змогла мутировати об'єкти по посиланню.

Отже, **side effects** виникають через **передачу посилань замість значень**.

_____________________________________________

Наступний приклад показує, що можна мутировати саму функцію-аргумент:
...
## ![ico-25 icon] Ідемпотентність⟪Idempotency⟫

Якщо метод при одному і тому ж значенні аргументів повертає один і той же результат, то він **_ідемпотентний_**.

^^Тобто при повторному виклику методу з одним і тим же масивом і функцією результат завжди буде однаковим.^^

У об'єктно-орієнтованих мовах ідемпотентність методів практично недосяжна, якщо аргументи функції є посилальним типом даних.
Оскільки масиви є посилальним типом, ітеруючі методи працюють із посиланням, а не з колекцією даних.
Посилання залишається тим самим, але між двома викликами методу вміст масиву міг змінитися, що позначиться на результаті.

Давайте спробуємо створити ідемпотентний метод:
...
При першому виклику методу **~idempotence~** з даним масивом метод мутує масив, додаючи йому властивість ~initialState~ і зберігаючи в цій властивості посилання на вихідний стан масиву.

При повторному виклику метод ігнорує всі зміни, які могли відбутися з масивом за час між першим і другим викликом методу, і працює з клоном ~initialState~.
...
Зауважимо, що при цьому кожного разу метод буде повертати нове посилання, оскільки при кожному виклику метод створює новий масив, проте вміст цього масиву буде однаковим.

Для чистоти експерименту можна повертати JSON-рядок властивості ~initialState~.
...
Однак у цьому прикладі ми використали ідемпотентний метод **~Math.sqrt~**.

Якщо ж ми передамо методу **~idempotence~** функцію, яка не є ідемпотентною, то метод не буде ідемпотентним, оскільки при виклику з одним і тим же набором аргументів він буде повертати різний результат:

~~~js
numbers.idempotence(item => item + Math.floor(Math.random() * 100))
~~~

У цьому прикладі видно, що якщо функція, яку ми передаємо методу **~idempotence~**, не є _ідемпотентною_, то і результат роботи методу буде варіюватися, тобто ідемпотентність методу безпосередньо залежить від ідемпотентності функції-аргументу.
...
Спробуйте самостійно розібратися в наступному прикладі.
...
Передача функції-аргументу другого (необов’язкового) параметра (індексу поточного елемента масиву).
...
Використовуємо другий і третій опицональні параметри функції-аргументу:
...
[◄◄◄Методи ітерації масивів◄◄◄](page/Array-iterating-methods.md)
