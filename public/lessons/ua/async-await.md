...
та ще дві допоміжні функції: **~sin~** і **~cos~**:
...
### ![ico-20 icon] Об'єкт з методом then⟪An_object_with_a_then_method⟫
...
### ![ico-20 icon] Згадаймо обіцянки⟪Let-s_recall_promises⟫
...
Отже, движок натрапляє на вираз ~**await** &lt;expression>~.
Йому потрібно обчислити значення цього виразу.
![ico-25 warn] Доки він не обчислить його та не замінить вираз ~await &lt;expression>~ на отримане (обчислене) значення, він не перейде до наступного рядка коду асинхронної функції.

Якщо значенням ~<expression>~ буде посилання на екземпляр **~Promise~**, то движок викличе метод **~then()~** цього екземпляра та передасть йому колбек:

~~~js
response => response
~~~
і призупинить виконання коду асинхронної функції до тих пір, поки колбек не повернеться з **Event Loop** з "кулям" результату (~response~) і не покладе його в коробку промиса.

Після того, як результат з'явиться в коробці, движок витягне його з коробки і вставить на місце виразу ~await &lt;expression>~.

Ми вже задалися питанням, що робитиме движок, якщо ~&lt;expression>~ не буде промісом.
І ми вже розібралися, що буде робити движок, якщо ~&lt;expression>~ буде об'єктом, у якого є метод **~then()~**.

Тепер подивимося, що робитиме движок, якщо ~&lt;expression>~ буде рядком, числом або іншим значенням.
...
У цьому прикладі потрібно уважно стежити за послідовністю виведення повідомлень у консоль:
...
Усе, що виводиться в консоль після повідомлення **_Finish_**, — це функції зворотного виклику, які «пройшли» через **Event Loop**.
Якщо вони виведені до ~undefined~ — це **microtasks**.

Отже, движок успішно вивів у консоль ^^**_Start_**^^, потім натрапив на функціональний вираз ([IIFE](page/Closure#IIFE)) і почав «обчислювати» значення виразу в круглих дужках. А в круглих дужках — оголошення анонімної асинхронної функції. Рушій передає управління конструктору, який створює цю функцію та повертає посилання на неї. Рушій, отримавши посилання на функцію, викликає її, оскільки далі йдуть круглі дужки (виклик функції).
Анонімна асинхронна функція починає виконуватися, і в консолі виводиться повідомлення ^^**_Async function starts_**^^.
Однак уже в наступному рядку коду функції движок «натрапив» на вираз:
~~~js
console.log(await 'Hello!')
~~~
Тут движок розуміє, що потрібно відправити колбек ~() => 'Hello!'~ у **Event Loop**, і функція повинна дочекатися його повернення, щоб завершити виконання цього рядка коду. Далі код функції не може виконуватися, поки не повернеться колбек. А повернутися він може лише тоді, коли стек викликів буде вільним.
Отже, движку потрібно на деякий час «позбутися» цієї функції, але так, щоб після повернення колбека ~() => 'Hello!'~ можна було відновити її виконання.
Як це можна зробити?

Наприклад, замінивши залишковий невиконаним код функції:
...
І далі движок продовжує виконувати код скрипта з рядка:

~~~js
console.log('Finish')
~~~

після чого стек викликів звільняється, і з **Event Loop** повертається зворотний виклик:
...
який отримав повідомлення з значенням 'Привіт!'.

Таким чином, поява ключового слова **~await~** призводить до того, що невиконаний залишок коду асинхронної функції перетворюється на зворотний виклик.
...
У цьому прикладі код основного потоку відпрацює при значенні **5** змінної ~num~.
...
1. Промис, повернений функцією ~async () => await 'async-await'~, розрізольвався останнім, навіть якщо це перший рядок коду.
2. Першим розрізольвався ~Promise.resolve('promise')~ (другий рядок коду).
3. Другим розрізольвався промис, повернений функцією ~async () => 'async'~ (третій рядок коду).

Зробивши "рокіровку" другого та третього рядків коду:
...
Ми побачимо, що порядок виведення в консоль змінився відповідним чином:
...
Тобто їхні колбеки потрапляють у чергу мікротасків у тій же послідовності, в якій вони з'явилися в коді.
...
Для цього запустимо їх спочатку в такій послідовності:
...
а потім поміняємо порядок їх слідування:
...
Як ми бачимо, вони розрізольвлюються строго в порядку їх слідування в коді.
Тобто поява ключового слова **~await~** подовжує ланцюжок колбеків, що проходять через **Event Loop**, на 1, що і призводить до того, що в прикладі 12 останнім "приходить до фінішу" ~;(async () => await 'async-await')().then(console.log)~.

Як ми вже говорили раніше, сама асинхронна функція переривається в точці, де зустрічається **~await~**, і залишок коду цієї функції, який не був виконаний до цього, сам стає колбеком і "йде" в **Event Loop**, щоб звільнити стек викликів і дати можливість колбеку повернутися з результатом. Тому ланцюжок подовжується на 1 мікротаск.
...
Зафіксуємо значення поточного часу в мілісекундах у змінній **~start~** та оголосимо допоміжні функції **~getRandom~** і **~test~**:
...
Як ви можете переконатися, повідомлення виводяться у випадковому порядку, залежно від того, яке значення повернула функція **~getRandom~** для кожного промісу.

Завдання - синхронізувати потрапляння колбеків у стек викликів так, щоб першим у консоль було виведено "**_First_**", потім "**_Second_**", а потім "**_Third_**".
...
Отже, асинхронна функція може слугувати «оболонкою» для кількох асинхронних операцій, виконання яких можна впорядкувати в часі, тобто зробити так, щоб їхні колбеки виконувалися у заданій послідовності.

________________________________

### ![ico-25 cap] 17⟪17⟫

Розглянемо суто гіпотетичний варіант
(на практиці так робити не слід):
...
методу **reduce** передається асинхронна функція, яка повертає проміс.
Тому після кожної ітерації змінна **result** буде промісом,
і її треба резолвити за допомогою **await**.

У результаті роботи скрипта у змінній **users** буде проміс.
Витягнемо результат із промісу:
...
Як бачимо, доданий елемент матиме два методи: **~addChar~** та **~replace~**, що дозволяють модифікувати текстовий вміст елемента.

Тепер розширимо прототип конструкторів **~String~** і **~Number~** методом **~then~**:
...
Тепер можна створити «простенькі» функції **~typeWritter~** та **~showNumber~**:
